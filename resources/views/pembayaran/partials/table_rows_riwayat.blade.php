...
@php
    ...
    // URL nota
    $notaUrl = route('admin.tagihan.nota', ['id' => $pay->id_pembayaran]);
@endphp
...
<tr>
    ...
    <td class="text-center">
        <div class="d-flex justify-content-center gap-1 flex-wrap">
            <button type="button"
                    class="btn btn-sm btn-success fw-bold btn-preview-nota-riwayat"
                    data-nota-url="{{ $notaUrl }}?embed=1">
                <i class="bi bi-eye"></i>
            </button>

            <a href="{{ $notaUrl }}?embed=1&print=1"
...
{{-- MODAL DETAIL TAGIHAN (tetap di partial ini boleh, karena id unik per pembayaran) --}}
                                ...
                                <table class="table table-sm table-bordered align-middle mb-0">
                                    ...
                                    @foreach($pay->items as $item)
                                        @php
                                            ...
                                            $bulanTahun = $tagihan
                                                ? \Carbon\Carbon::create($tagihan->tahun, $tagihan->bulan, 1)->translatedFormat('F Y')
                                                : '-';
                                        @endphp
                                        <tr>
                                            <td class="text-center">
