...
{{-- ATAS: SISA BULAN LALU --}}
<div class="d-flex justify-content-between align-items-center">
    <div>
        <small class="text-muted d-block">Sisa Bulan Lalu</small>
        @php
            $sisaClass = $sisaBulanLalu > 0 ? 'text-danger' : ($sisaBulanLalu < 0 ? 'text-success' : 'text-muted');
        @endphp
        <div class="fw-semibold {{ $sisaClass }}">
            Rp {{ number_format(abs($sisaBulanLalu), 0, ',', '.') }}
            <small class="ms-1">
                ({{ $sisaBulanLalu > 0 ? 'Kurang' : ($sisaBulanLalu < 0 ? 'Lebih' : 'Pas') }})
            </small>
        </div>
    </div>
</div>
...
<div class="d-flex justify-content-between">
    ...
    <div class="text-end">
        <small class="text-muted d-block">Sudah Disetor Bulan Ini</small>
...
<div class="mt-2 small">
    @php
        $stClass = $statusBulanIni === 'lebih' ? 'text-success' : ($statusBulanIni === 'kurang' ? 'text-danger' : 'text-muted');
    @endphp
    <span class="fw-semibold {{ $stClass }}">
        Status Bulan Ini: {{ strtoupper($statusBulanIni) }}
    </span>
    <span class="text-muted">
        (Selisih: Rp {{ number_format(abs($selisihBulanIni), 0, ',', '.') }})
    </span>
</div>
...
<div class="quick-stats">
    ...
    <!-- BARIS 4 (FINAL: 2 KATEGORI) -->
    <a href="{{ route('seles2.pelanggan.statusBayar', ['status_bayar' => 'lunas']) }}" class="stat-card">
        <div class="stat-icon success">
    ...
    <a href="{{ route('seles2.pelanggan.statusBayar', ['status_bayar' => 'belum']) }}" class="stat-card">
        <div class="stat-icon danger">
